...
export default {
  ...
  methods: {
    ...
    try {
      ...
      const room_res = await callApi.GetRoomTeacher(req);

      const classValue = room_res.result.class;
      const room = room_res.result.room;
    ...
    try {
      ...
      const room_res = await callApi.GetRoomTeacher(req_room);
  ...
  processApiData(data) {
    ...
    useExampleData() {
      ...
      this.roomInfo.name = '<span style="color: red;">ไม่พบข้อมูล</span>';
