<template>
  <div class="container mt-4 p-4 mb-5 bg-white shadow rounded">
    ...
    <p class="text-center fs-6">
      โปรดเลือกคำตอบในช่องที่ท่านเห็นว่าตรงกับท่านมากที่สุด
    </p>
      ...
      <table class="table table-bordered table-striped table-hover table-fixed-header">
        ...
        <tbody class="fm-IBM">
          <tr v-for="q in questions" :key="q.id">
            ...
            <td class="text-center centerButton">
              <label class="container2 ">
              <input type="radio" :name="'q' + q.id" :value="q.values[3]" v-model="q.value">
              <span class="checkmark"></span>
              </label>
            </td>
          </tr>
    ...
    <div class="d-flex justify-content-center gap-3">
      <button class="btn btn-primary fs-5" @click="submitForm">ส่งแบบประเมิน</button>
  ...
  export default {
    ...
    methods: {
      ...
      async submitForm() {
        ...
        console.log(res.result[0].eq_status);
        if(res.result[0].eq_status){
  ...
  .sticky-header {
  position: sticky;
  top: 5; /* ติดอยู่ที่ด้านบนของหน้าจอ */
